...
class RoutingContext(private val _asJava: Object) {
   ...
   * e.g. create a failure response page.   * @return the throwable used when signalling failure
   */
   ...
   * match will be en-US because it was declared as first entry by the client.   * @return the best matched locale for the request
   */
   ...
   * When 2 or more entries have the same quality then the order used to return the best match is based on the lowest
   * index on the original list. For example if a user has en-US and en-GB with same quality and this order the best
   * match will be en-US because it was declared as first entry by the client.   * @return The best matched language for the request
   */
   ...
   * locales.   * @return the users preferred locale.
   */
   ...
   * It is the same action as returning the first element of the acceptable languages.   * @return the users preferred locale.
   */
   ...
   * and available on the params list.   * @param path the new http path.
   */
   ...
   * if accessing server resources requested by a client.   * @return the normalised path
   */
   ...
   * [[io.vertx.scala.ext.web.handler.BodyHandler]] for this to be populated.   * @param encoding the encoding, e.g. "UTF-16"
   * @return the body
   */
   ...
   * Sessions live for a browser session, and are maintained by session cookies.   * @return the session.
   */
   ...
   * matches one or more of these then this returns the most acceptable match.   * @return the most acceptable content type.
   */
   ...
   * you can write any extra headers before the response has been written when it will be too late.   * @param handler the handler
   ...
   * Do not use this for resource cleanup as this handler might never get called (e.g. if the connection is reset).   * @param handler the handler
   * @return the id of the handler. This can be used if you later want to remove the handler.
   */
   ...
   * params list.   * @param method the new http request
